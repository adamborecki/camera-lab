...
4. **Depth of field** — thin-lens circle of confusion per layer, from focal length, f-number, focus distance and sensor size. At a matched framing a smaller sensor runs a shorter lens, so it gets deeper focus for free; at the same real lens it sees the same blur, just cropped and magnified. Blur is computed in scene pixels (the view magnifies it afterwards), and blurred layers are cached and only re-blurred when focus/aperture/lens/camera change.
