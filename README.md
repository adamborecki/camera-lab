...
3. **Depth of field** — thin-lens circle of confusion per layer, from focal length, f-number, focus distance and sensor size. Scenes set the framing as a 35mm-equivalent focal length, so smaller sensors get shorter lenses and deeper focus automatically. Blurred layers are cached and only re-blurred when focus/aperture/camera change.
