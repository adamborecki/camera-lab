...
3. **Depth of field** — thin-lens circle of confusion per layer, from focal length, f-number, focus distance and sensor size. Scenes set the framing as a 35mm-equivalent focal length, so smaller sensors get shorter lenses and deeper focus automatically. "Acceptably sharp" is a blur circle under `SHARP_PX` (2) pixels of the 1280-wide frame — the usual 0.03 mm-on-full-frame criterion is a print standard that works out to ~1 px here, finer than the picture can draw — so the focus map's in-focus band lands exactly where visible blur begins. Blurred layers are cached and only re-blurred when focus/aperture/camera change.
4. **Motion blur** — moving layers are integrated over the time the shutter is actually open (sub-frame samples), and the picture updates at the chosen frame rate. Shutters slower than a frame repeat frames, like camcorder slow shutter.
5. **Sensor** — photon shot noise + read noise (sensor-size dependent), per-channel clipping, then ISO/dB gain (so gain amplifies noise), white balance as RGB gains from kelvin (blackbody), highlight desaturation, a Rec.709-ish tone curve with a shoulder, sRGB out.
6. **Monitoring** — histogram/waveform read back the rendered frame (before zebras/peaking overlays), so scopes always agree with the picture.
...
- Values are simplified for teaching; relationships (stops, angles, crop) are real.
